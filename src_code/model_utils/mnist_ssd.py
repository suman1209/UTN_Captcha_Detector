...
class MultiBoxLossSSD(nn.Module):   
    ...
    def forward(self, loc_output, cla_output, boxes, labels):
        ...
        for i in range(len(boxes)): #for each image in batch
            ...
            # print(cla_gt[i].size(), (max_overlap_for_each_prior < self.configs.pos_box_threshold).size())
        ...
        #     debug_info["hardneg_default_boxes"] = [self.priors_xy[hard_negatives[i]] for i in range(hard_negatives.size(0))]
        ...
        # logger.log({"ce_pos_loss": ce_pos_loss, "ce_neg_loss": ce_neg_loss})
        if self.debug:
